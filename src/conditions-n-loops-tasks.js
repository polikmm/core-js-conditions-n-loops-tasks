...
function isPalindrome(str) {
  ...
  for (let i = 0; i < middle; i += 1) {
    for (let j = str.length - 1; j > middle; j -= 1) {
...
function getSpiralMatrix(size) {
  ...
  while (colStart <= colEnd && rowStart <= rowEnd) {
    ...
    for (let k = colEnd; k >= colStart; k -= 1) {
      matrix[rowEnd][k] = num;
      num += 1;
    }
    ...
    for (let m = rowEnd; m >= rowStart; m -= 1) {
...
function shuffleChar(str, iterations) {
  ...
  while (isChanged) {
    ...
    for (let i = 0; i < copy.length; i += 1) {
      if (i % 2 === 0 || i === 0) left = `${left}${copy[i]}`;
      if (i % 2 !== 0) right = `${right}${copy[i]}`;
    }
    copy = `${left}${right}`;
    cache[count] = copy;
    count += 1;
    if (copy === str) isChanged = false;
  }

  if (iterations === count) return copy;
  if (iterations < count) return cache[iterations - 1];
  return cache[(iterations % count) - 1];
}
...
function getNearestBigger(number) {
  ...
  for (let j = num.length - 1; j >= i; j -= 1) {
    if (num[i - 1] < num[j]) {
      const temp = num[i - 1];
      num[i - 1] = num[j];
      num[j] = temp;
  ...
  return +[...num.splice(0, i), ...num.splice(0).sort()].join('');
}
